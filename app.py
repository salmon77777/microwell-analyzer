...
# 가로/세로 간격도 직접 입력(number_input)으로 변경하여 정밀도 향상
    ...
    for r in range(row_count):
        for c in range(col_count):
            ...
            if center_x < w and center_y < h:
                ...
                # 판정에 따른 테두리 색상 (BGR 기준: Blue=(255,0,0), Red=(0,0,255))
                if green_val > threshold:
                    pos_count += 1
                    border_color = (255, 0, 0) # Positive (파란색)
                else:
                    border_color = (0, 0, 255) # Negative (빨간색)
